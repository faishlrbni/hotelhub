...
export function Sidebar({ mobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  const renderNavBody = (isMobile = false) => (
    <div className="h-full flex flex-col justify-between overflow-y-auto">
      ...
      <div>
        {/* Logo Header */}
        <div
          className={`h-16 flex items-center justify-between border-b border-black/[0.04] dark:border-white/[0.06] ${
            !isMobile && collapsed ? 'justify-center px-2' : 'px-5'
          }`}
        >
          <Link
            href="/dashboard"
          ...
          {isMobile && (
            <button
              ...
              style={{ borderRadius: '50%' }}
              className="w-8 h-8 flex items-center justify-center text-[var(--text-tertiary)] hover:text-[var(--text-primary)] bg-[var(--bg-left-panel)] transition-colors cursor-pointer"
            >
              <X className="w-4 h-4" />
            </button>
          )}
        ...
        <nav className="p-3 space-y-1.5 overflow-y-auto max-h-[calc(100vh-210px)]">
          {NAV_ITEMS.map((item) => {
            ...
            const isItemCollapsed = !isMobile && collapsed;
            ...
            return (
              <Link
                ...
                className={`flex items-center font-medium text-xs transition-all duration-200 group active:scale-[0.97] ${
                  isItemCollapsed
                    ? 'justify-center w-10 h-10 mx-auto p-0 hover:scale-105'
                    : 'gap-3 px-3.5 py-2.5 hover:translate-x-1'
                } ${
                  isActive
  ...
  return (
    <>
      ...
      <aside
        className={`hidden md:flex h-screen sticky top-0 bg-[var(--bg-card)] border-r border-black/[0.06] dark:border-white/[0.08] flex-col justify-between transition-all duration-300 z-30 relative shrink-0 ${
          collapsed ? 'w-20' : 'w-64'
        }`}
      >
      ...
      {/* MOBILE DRAWER OVERLAY (Visible on mobile when mobileOpen is true) */}
      {mobileOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          {/* Drawer Sheet */}
          <aside className="relative w-72 max-w-[85vw] h-full bg-[var(--bg-card)] shadow-2xl border-r border-black/[0.08] dark:border-white/[0.12] flex flex-col justify-between z-50 animate-in slide-in-from-left duration-250">
            {renderNavBody(true)}
          </aside>
        </div>
      )}
